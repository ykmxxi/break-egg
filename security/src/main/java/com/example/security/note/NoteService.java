...
import com.example.security.user.UserNotFoundException;
...
public class NoteService {
    ...
    public List<Note> findByUser(final User user) {
        if (user == null) {
     ...
     * @return 저장된 노트
     */
    public Note saveNote(final User user, final String title, final String content) {
        if (user == null) {
            throw new UserNotFoundException();
        }
     ...
     * @param noteId 노트 ID
     */
    public void deleteNote(final User user, final Long noteId) {
        if (user == null) {
            throw new UserNotFoundException();
        }
